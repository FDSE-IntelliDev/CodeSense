...
from codesense.config import BASE_MODEL
from codesense.dsl.surface_con import surface_condition
from codesense.dsl.intention_con import intention_condition
from codesense.dsl.relation_con import relation_condition
...
4. Every list item must strictly follow its corresponding schema above. Do not add fields outside the corresponding schema.
5. If a category has no conditions, use an empty list.
6. Surface group_logic pair rules:
   - group_logic only describes graph-aware AND among include keyword groups; never put exclude groups in it.
   - Every group_logic item is exactly one unordered pair rule. Its groups field must contain exactly two distinct group_id values that exist in keyword_groups and both have property="include".
   - hop_count must be a non-negative integer and is the inclusive maximum graph distance allowed between direct hits from the two groups. hop_count=0 requires the same code element to satisfy both groups.
7. We will use surface for literal/code-text matching, intention for behavior/intent/domain meaning, and relation for code structure constraints.
8. CRITICAL — match_kind atomicity: Each surface condition must have exactly ONE match_kind value (code_element / code_snippet / code_line / unknown). Do NOT combine multiple match_kinds into a single surface condition. If the query involves matching more than one kind (e.g., both a code element name and a code line), split them into separate surface conditions, each with its own match_kind and corresponding keyword_groups.
