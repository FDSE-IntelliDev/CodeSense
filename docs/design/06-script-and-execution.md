...
| | grounded（单单元） | graph（+图重排） | **planned（全流程）** |
|---|---|---|---|
| 平均 | 54% | **65%** | **21%** |

**这是个确凿的负面结果，不是噪音**——三次采样一致。诊断出四个原因，
前三个已修，第四个是设计层面的：
...
4. **单元分解本身摊薄了信号。** 一个覆盖 38% 代码库的单元和只覆盖 1% 的
   单元等权相加，真正的答案只强命中窄单元，却输给在宽单元里刷了三个词的噪音。
   按特异性加权试过，没有改善。

### 这对设计意味着什么

第 4 条指向 [08](08-open-questions.md) 的 Q2（单元之间默认什么关系），
而且答案比预想的更不利：**在当前的打分模型下，把查询拆成多个单元
不如把词堆进一个单元**。

可能的方向（都还没验证）：

- 分数不该跨单元相加，而应按单元取 `max` 再合成——避免宽单元靠数量取胜
- 单元只在有图约束把它们连起来时才拆；没有图约束就别拆
- 让编译回路（本章上半部分那个「试跑 → 太少放宽 / 太多收紧」）真正跑起来，
  用实际规模而不是预估来决定要不要拆

**在这些验证之前，不该声称查询单元这个抽象是划算的。**
它在概念上是对的（图约束确实要挂在单元上），但目前的实现里它是净损失。
