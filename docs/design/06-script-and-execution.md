...
编译器已实现（`codesense/ql/compile/`），**产物就是脚本**（`to_script()`）。

中间隔了一层 `Plan`：规划、代价估计、预估对比实际都在它上面做，
脚本是它的渲染结果。**两者必须等价**——
`tests/unit/ql/compile/test_emit.py` 会真的跑一遍生成的脚本，
断言它和执行 `Plan` 得到同一个符号集合。

这条测试不是形式主义。写它之前踩过两次：`only(kind=...)` 被渲染成硬过滤
而计划里是偏好加权；`Cohere` 在脚本里当场重排而计划里只标记邻域。
两次都是脚本比计划筛得更狠——**一份看着像那么回事、行为却不同的脚本，
比没有脚本更糟**，因为人会照着它推断系统行为。

安全性上的取舍不变：脚本由编译器渲染、模板固定，模型只填词和判定标准，
不生成代码结构。
...
### 两条路：结构化流水线 vs 直接生成脚本

后来发现这个问题本质上可以是个**代码生成**问题：QL 建在 Python 上，
把算子 spec 和**带 `df` 的词表**一起给模型，它就能自己写脚本。

结构化流水线那条路建立在「模型不知道 `buffer` 在 netty 里命中 2365 个符号」
之上——但那是**提示词设计问题，不是架构必然**。词表带上统计一起给它，
它就拥有和规划器一样的依据。
...
| | 均值 | 代码量 |
|---|---|---|
| 结构化流水线（提议→校验→规划→渲染） | 51% | 1,539 行 |
| **直接生成脚本** | **56%** | **354 行** |
| 手写编排（参照） | 59% | — |

**用四分之一的代码拿到更好的结果，且已逼近手写。**
结论是结构化流水线那五层中间表示（QuerySpec → 分区 → 校验 → Plan → 渲染）
是过度设计：每一层都是一次信息损耗，而它们要防的问题，
把统计喂给模型就解决了。

首次尝试只有 9%，但失败全在生成脚本的**逻辑**上，而且都很具体：
模型只挑了 4 个词、把 `reach` 当替换而非加权用（词法分数全丢）、过早截断。
在 spec 里补一段范例并写明「`reach`/`hop` 的产物不带分数」之后就到了 56%。
**这些是提示词问题，不是能力上限。**

> 中间还踩过一次自己挖的坑：白名单里列了 `set` 却没把它放进执行环境，
> 生成的脚本完全正确却因 `NameError` 全军覆没（0%）。
> 白名单和实际提供的东西必须**同源**。

### 控制流：放行，用预算而非禁令约束

最初的闸门禁掉了 `for` / `while`，理由是「编译产物里没有正当用途」。
**这掐掉了选择 Python 脚本的初衷**——中间变量、条件分支、迭代收敛，
算子因此能编排成计算图而不只是一条直线；本章上半部分那个
「试跑 → 太少放宽」的回路，本来就可以直接写进脚本里。

现在控制流放行，靠**执行预算**防不终止（`sys.settrace` 只追踪脚本自身的帧，
算子内部不计入，否则一次 `eval_unit` 就能耗光预算）。
调用方注入命名空间的名字一律可调用——命名空间里有什么本来就是调用方决定的，
白名单管的是「能不能碰到注入之外的东西」。

> **但放开之后召回率没变**（52% vs 56%，噪声内），
> 而且生成的 8 个脚本**一个都没用控制流**。
> 原因大概率是 benchmark 本身：8 条查询都是单概念，一个单元加一次重排就够了，
> 真的用不上分支和迭代。**这个能力该有，但它的价值这套查询证明不了。**

### 分工：模型提议，统计校验

这是走了两个极端之后定下来的。

**极端一**：让模型一次性给出整份规格——单元怎么分、偏好什么种类、
要不要图约束，全由它定。**R@100 从 65% 掉到 21%。** 根因是模型在做
它**没有依据**做的决定：它不知道 `buffer` 在 netty 里命中 2365 个符号，
不知道这条查询的答案是 field 不是 method。

**极端二**：不让模型碰任何结构，全部由统计推。回到 47%，
但**完全用不上图**——图约束没人提议了。
...
| 阶段 | 谁做 | 例子 |
|---|---|---|
| **提议** | LLM | 哪些词相关、按语义怎么分组、有没有「A 调用 B」这层意思 |
| **校验** | 统计 | 组内的词真落在同一批符号上吗？这个关系在本项目里成立吗？ |
| **参数化** | 统计 | 偏好什么种类、从哪一侧出发、走几跳、什么顺序 |

关系校验用**边密度相对随机图的倍数**，判别力很清楚（netty 实测）：
...
分组校验用组内两两 Jaccard 的均值。模型按**语义**分组，
但语义相近不代表落点相同——落点不同的词凑一组，等权相加就把答案摊薄了。

### 图约束 ≠ 图邻近性

实践中发现这两个概念被我混在了一起，分开之后才把最后一块补上：

| | 什么时候适用 | 谁决定 |
|---|---|---|
| **图约束**（`Boost`） | 查询语义里真有「A 相关的代码调用 B」——**大部分查询没有** | 模型提议 + 统计校验 |
| **图邻近性**（`Cohere`） | 「离强命中近的更可能也是答案」——**每条查询都成立** | 纯统计，不问模型 |

模型对「堆外内存的申请与释放」只提 1 个组、0 条关系，这是**对的**——
那条查询本来就只讲一件事。但它仍然受益于图，因为受益的是邻近性不是约束。
把 `Cohere` 无条件加进计划之后，R@100 从 43% 回到 49%。
...
`planned` 49% vs `graph` 59%。剩下的差距还没定位清楚，
候选原因是词的权重处理：手工那条路把 25 个词平权丢进一个
`LexicalSatisfier`，全流程则按模型给的相关度加权。
**这条要下次验证，现在不该猜。**
