...
`Element(kind="file")` 是索引和图中的真实节点，不是把声明结果按 `file` 字段去重后的
展示形式。每个成功扫描的源文件都有一个文件节点，包括没有可索引声明的空文件；它沿用
普通 `Element` / `Hit` 的字段和序列化契约。

查询的 `target=("file",)` 是最终结果类型的硬约束：结果必须是文件节点，非文件候选需要
沿图投影到文件。`QuerySpec.kinds` 与 `Narrow.kind` 仍只是候选和排序偏好，不能替代
`target`。未指定 target 时保持兼容行为，文件节点不会混入普通声明结果。

文件节点当前用于结果目标和关系组合，不产生路径 postings。直接按路径或 glob 检索文件、
以及对任意文件内容执行正则搜索仍不支持；这些能力需要独立的路径/内容索引，不能从
`Element.file` 字段或 file target 推导出来。
...
| `in_file` | 声明物理上属于文件 | 「包含这些声明的文件」 |
| `references` | A 引用 B（通用关系） | 「哪些元素引用了这个类型」 |
...
```python
f.roots()      # 入度为 0 的节点（路径起点，没有边指向它们）
f.leaves()     # 出度为 0 的节点（路径终点）
