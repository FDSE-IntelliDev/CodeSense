...
```python
def hop(src, dst, *, edge, dir, len, via, avoid, min_confidence, max_paths):
    lo, hi = _len_range(len)
    # 从 src 正向、从 dst 反向同时推进，在中点相遇
    # 每层记 pred[node] = [(prev_node, edge_id), ...]，而不是只记 visited
    # 相遇后回溯前驱链拼出完整路径
```

双向的理由不是常数优化：单向 BFS 到深度 `d` 要展开 `b^d` 个节点，
双向是 `2·b^(d/2)`。按实测修复后平均度 4.4：

```
len=(1,3)   单向 ~109 条/起点      双向 ~2·4.4^1.5 ≈ 18
len=(1,5)   单向 ~2133 条/起点     双向 ~2·4.4^2.5 ≈ 81
```
...
3. **`len` 的上界要小。** 默认 `(1, 3)`。跳数越多，「有关系」这个结论越弱——
...
6. **`hop` 的路径爆炸要四道闸**：`max_paths` 默认值 + 截断日志、
   hub 节点限流、`len` 上界小、`avoid` 有默认值。
   实测平均度修复后约 4.4，`len=(1,5)` 单向就是 2133 条路径/起点。
