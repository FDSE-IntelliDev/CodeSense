...
## 八、benchmark 实测：图确实有用，而且不必上 CodeQL 才有用

在 netty（42221 符号）、HikariCP、spring-petclinic 上跑语义检索 benchmark，
三次独立采样的 R@100 平均：

| | literal | generic | grounded | **graph** |
|---|---|---|---|---|
| 平均 | 0% | 39% | 56% | **61%** |

`graph` 是在 `grounded` 的词法结果上做图重排序：取最强的 20 个命中当种子，
沿 `calls` / `contains` 走 1~2 跳，落在邻域里的候选乘性加成。
**三次采样里它都是正向的**（+6 / +5 / +5 个百分点），
netty 的零拷贝查询从 75% 提到 100%。

### 不建库也能拿到大部分调用边

CodeQL 对 netty 意味着跑完整 `mvn compile`。先用 AST 里免费的类型信息：

```
接收者表达式        解析方式                        置信度
this.m() / m()     所属类 + 父类型                  0.9
field.m()          字段的声明类型                    0.9
local.m()          局部变量/参数的声明类型            0.9
Type.m()           接收者本身就是类型名               0.9
obj.a().b()        认不出来 → 退回按名字匹配          ≤0.6
```
...
|---|---|---|
| 高置信度边占比 | 13% | **44%** |
| 类型解析覆盖的调用点 | 0 | 48379（60%） |
| 因名字太泛丢弃 | 25147 | 12380 |

**所以 CodeQL 不是「有没有图」的分界线，是「图有多准」的分界线。**
它仍然不可替代的是：虚方法分派的完整实现集、泛型、跨库调用、
以及 `flows_to`。但那些应当在**基础图已经有用之后**再投入。

### 方法论：LLM 的方差和效应同量级

同一条查询、同一份词表、`temperature=0`，两次运行的召回率能差 20 个百分点。
所以 benchmark 必须**缓存派生结果**，并跑多次采样报均值——
否则任何跨运行的对比都是在读噪音。这一条踩过一次才发现。
...
6. **benchmark 证实图有用，而且不必上 CodeQL 才有用。** 图重排让 R@100
   从 56% 升到 61%（三次采样一致正向）。而 AST 里免费的接收者类型
   就能把高置信度边从 13% 提到 44%——CodeQL 是「图有多准」的分界线，
   不是「有没有图」的分界线。

7. **`hop` 的路径爆炸要四道闸**：`max_paths` 默认值 + 截断日志、
